...
export const Helm: FC = () => {
  const { mutateAsync: helmMutate, isPending: helmPending } = usePost<
    HelmResponse,
    HelmBody
  >(API.Helm, 'helm-template', true);
  ...
  const methods = useForm({
    resolver: zodResolver(helmSchema),
    defaultValues,
  });
  ...
  const handleSubmit = async (data: HelmSchema) => {
    try {
      ...
      await helmMutate(body);
      await download({ fileName: 'HelmTemplate.zip' });
    } catch (error) {
      console.log(error);
      if (isAxiosError<helmValidationError>(error)) {
        toast.error(
