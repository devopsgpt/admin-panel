...
import {
  argoWorkflowsSchema,
  ArgoWorkflowsSchema,
} from './argo-workflows.types';

export function ArgoWorkflows() {
  const getServices = useGet<string[], unknown>(
    '/argo-workflows-get-services',
    'argo-workflows-get-services',
    false,
  );
  ...
  const getServicesMethod = useForm<ArgoWorkflowsSchema>({
  ...
  async function handleSubmitService(body: ArgoWorkflowsSchema) {
    try {
      setGetServiceTemplatePending(true);
      const { data } = await externalTemplateInstance.get(
        `/argo-workflows-get/${body.service.value}`,
        { responseType: 'blob' },
      );
